...
function initializeGame() {
    if (document.getElementById("hp").value == 0) { // checks if you have already won or lost
        lose()
        return;
    }
...
function animateDrawCard(slotIndex, type) {
    ...
    card.addEventListener("click", () => {
        console.log(type)
        playCard(type)
        let e_dmg = Math.floor(Math.random() * max_e_dmg) + 1
        gethit(e_dmg)
    })
...
function playCard(cardName) {
    if (cardName == "Attack") {
        ...
        gethit(-8)
    } else if (cardName == "Buff") {
        player_dmg *= 1.2
    } else if (cardName = "Debuff") {
        max_e_dmg *= 0.9
        max_e_dmg = Math.round(max_e_dmg)
    }
}
...
function hit_e(dmg) {
    ...
    hp -= dmg //enemy takes damage
    if (hp > 100) hp = 100
    if (hp < 0) hp = 0
    if (hp <= 40) max_e_dmg += 2
    document.getElementById("e_hp").value = hp //save health in form for saving
    console.log("enemy hp is: " + hp) //log health
    document.getElementById("display_e_hp").textContent = hp //visualize health
...
function lose() {
    ...
    screen.style.pointerEvents = "auto" //so you cant click buttons/cards through the overlay
    screen.style.opacity = 100 + "%" //makes the overlay appear (it has transition)
    let s_btn = document.createElement("button")
    s_btn.addEventListener("click", () => {
        document.getElementById("save").click() //save/continue button on the overlay so you can save if you won/lost
        console.log("lost") // :(
    })
    screen.textContent = '"you lost"'
    screen.appendChild(s_btn) //button shown on overlay and lose text/button text
...
function win() {
    document.getElementById("done").value = 1 // marks game as complete
    let screen = document.getElementById("fadeOverlay")
    screen.style.pointerEvents = "auto" //same thing, overlay appear and sizing
    screen.style.opacity = 100 + "%"
    let s_btn = document.createElement("button")
    s_btn.addEventListener("click", () => { //same thing, button for saving on the overlay
